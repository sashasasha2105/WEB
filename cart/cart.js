...
let mapInstance = null, cityClusterer, postamatClusterer;
let currentCity = '', cityCode = null;

// === Init ===
document.addEventListener('DOMContentLoaded', () => {
  // Лоадер для карты и меток
  ...
  panel.id = 'pvz-info-panel';
  document.querySelector('.map-wrapper').appendChild(panel);

  // Подгружаем config.js, потом инициализируем всё
...
function initCartControls() {
  document.querySelectorAll('.plus-btn').forEach(btn =>
      btn.addEventListener('click', () => { counts[btn.dataset.id]++; saveCart(); updateUI(); })
  );
  ...
  document.querySelectorAll('.remove-item-btn').forEach(btn =>
      btn.addEventListener('click', () => { counts[btn.dataset.id]=0; saveCart(); updateUI(); })
  );
...
function initCitySuggest() {
  cityIn().addEventListener('input', debounce(async e => {
    ...
    clearClusters(); hideMap(); deliverySection().style.display='none';
    citySug().innerHTML='';
...
function initDeliveryToggle() {
  document.getElementById('deliveryPvz').addEventListener('change', () => {
    ...
    // сначала подгружаем саму карту
    showMap(currentCity, async () => {
      // затем все метки
      await fetchAndPlotPvz();
      // добавляем заглушку-панель
      const pnl = infoPanel();
      pnl.textContent = 'Тут должно быть описание – нажмите на ПВЗ';
      pnl.classList.add('placeholder','active');
  ...
  document.getElementById('deliveryCourier').addEventListener('change', () => {
    ...
    pnl.classList.remove('active','placeholder');
  });
}

// === Fetch & plot ALL types ===
